...
df_cmv["Motif"] = df_cmv["CDR3b"].apply(classify_motif)

# حساب frequency لكل motif
human_freq = df_human["Motif"].value_counts(normalize=True)
cmv_freq = df_cmv["Motif"].value_counts(normalize=True)

# دمج الاتنين
df_motif = pd.DataFrame({"Human_freq": human_freq, "CMV_freq": cmv_freq}).fillna(0)

# حساب log2 enrichment
df_motif["log2_enrichment"] = np.log2((df_motif["CMV_freq"]+1e-9) / (df_motif["Human_freq"]+1e-9))
...
plt.xlabel("Motif")
plt.title("Motif Enrichment in CMV relative to Human baseline")
plt.tight_layout()
plt.show()
